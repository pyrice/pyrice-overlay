...
# -r2: propagate sycl to that ggml dep. Without it, llama-cpp[sycl] could be
# satisfied by a ggml built without sycl — which silently yields a Vulkan-only
# binary even though USE=sycl is on, since the backend library is ggml's to
# build, not llama-cpp's.
COMMON_DEPEND="
	>=sci-ml/ggml-0.17.0:=[openmp?,vulkan?,sycl(-)?]
	blas? (
		blis? ( sci-libs/blis:= )
...
RDEPEND="
	${COMMON_DEPEND}
	server? ( dev-libs/openssl:= )
	sycl? ( elibc_glibc? ( dev-libs/intel-compute-runtime[l0] ) )
"
