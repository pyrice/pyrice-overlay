...
# FindBLAS.cmake: Fortran enabled → BLAS_mkl_THREADING=gnu → sci-libs/mkl[gnu-openmp]
#                 Fortran disabled → BLAS_mkl_THREADING=intel → sci-libs/mkl[llvm-openmp]
COMMON_DEPEND="
	blas? (
		blis? (
			sci-libs/blis:=
...
PATCHES=(
	...
	"${FILESDIR}/${P}-make-installing-runtime-deps-optional.patch"
)
...
src_prepare() {
	...
	if use rocm; then
		# --hip-version gets appended to compile flags, which is not a known flag.
		# This causes rocm builds to fail because -Wunused-command-line-argument is on.
