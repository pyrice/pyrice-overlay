...
src_install() {
	...
	# -r1: fperms with an unquoted glob expands against the live root, not
	# ${ED}, so it only matched when a previous install already provided the
	# same file names — first installs died here. Glob under ${ED} instead.
	local f
	for f in "${ED}"/usr/lib/ollama/libllama-*-impl.so \
		"${ED}"/usr/lib/ollama/lib*.so.0.0.0 \
		"${ED}"/usr/lib/ollama/llama-*; do
		[[ -e ${f} ]] || die "fperms glob matched nothing: ${f}"
		fperms 0755 "${f#"${ED}"}"
	done
